...
namespace FoxyMC
{
    ...
    public class MapGenerator
    {
        ...
        public bool GenerateMap(Level Lvl, string type)
        {
            ...
            {
                Server.s.Log("Gen Fail : " + e.ToString());
                Inuse = false;
                return false;
            }
        ...
        void GenerateFault(float[] array,Level Lvl, string type, Random rand)
        {
            ...
            Server.s.Log("Irrerations = " + numIterations.ToString());
        ...
        float Noise(int x, int y, int seed)
        {
            int n = x + y * 57 + seed;
            n = (n << 13) ^ n;
            unchecked { return (float)(1.0 - ((n * (n * n * 15731 + 789221) + 1376312589) & 0x7fffffff) / 1073741824.0); }  //This is a psudo random number gen
        ...
        void FilterAverage(Level Lvl)
        {
            ...
            for (int bb = 0; bb < terrain.Length; bb++)
            {
                ushort x = (ushort)(bb % Lvl.width);
                ushort y = (ushort)(bb / Lvl.width);
                filtered[bb] = GetAverage9((short)x, (short)y, Lvl);
            }
            Server.s.Log("Copying filtering");
            for (int bb = 0; bb < terrain.Length; bb++)
            {
                terrain[bb] = filtered[bb];
            }
            Server.s.Log("Completed filtering");
        }

        //Averages over 5 points
        float GetAverage5(short x, short y, Level Lvl)
        {
            divide = 0.0f;
            float temp = GetPixel(x, y, Lvl);
            temp += GetPixel((short)(x + 1), y, Lvl);
            temp += GetPixel((short)(x - 1), y, Lvl);
            temp += GetPixel(x, (short)(y + 1), Lvl);
            temp += GetPixel(x, (short)(y - 1), Lvl);

            return temp / divide;
        }
        //Averages over 9 points
        float GetAverage9(short x, short y, Level Lvl)
        {
            divide = 0.0f;
            float temp = GetPixel(x, y, Lvl);
            temp += GetPixel((short)(x + 1), y, Lvl);
            ...
            temp += GetPixel((short)(x + 1), (short)(y + 1), Lvl);
            temp += GetPixel((short)(x - 1), (short)(y + 1), Lvl);
            temp += GetPixel((short)(x + 1), (short)(y - 1), Lvl);
            temp += GetPixel((short)(x - 1), (short)(y - 1), Lvl);
        ...
        float GetPixel(short x, short y, Level Lvl)
        {
            if (x < 0) { return 0.0f; }
        ...
        //Forces the edge of a map to slope down for island map types
