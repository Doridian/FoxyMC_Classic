...
namespace FoxyMC
{
	...
	public class Level
    {
        ...
        public int physics
        {
            ...
            set
            {
                if (value > 0 && m_physics <= 0)
                {
                    physicsThread = new Thread(new ThreadStart(PhysThreadFunc));
                    this.physicsThread.Start();
                }
                else if (value <= 0 && m_physics > 0)
                {
        ...
        public string[, ,] plychanges;
        public byte[, ,] plyblocks;
        ...
        public Level(string n, ushort x, ushort y, ushort z, string type)
        {
            ...
            //width = (ushort)(width - (width % 32));
            depth = (ushort)(depth - (depth % 16));
            //height = (ushort)(height - (height % 32));

            if (width * depth * height > 214783640)
            {
                width = 512;
                depth = 512;
                height = 512;
            ...
            plychanges = new string[width, depth, height];
            plyblocks = new byte[width, depth, height];
            ...
            physicsThread = new Thread(new ThreadStart(PhysThreadFunc));
            if (this.physics > 0) physicsThread.Start();

            Server.s.Log("Level \"" + this.name + "\" initialized");
            this.Backup();

            Gamemode.Load(this);
        }

        private void PhysThreadFunc()
        {
            int msTimeout = 250;
            while (true)
            {
                ...
                try
                {
                    ...
                    this.CalcPhysics();
                    TimeSpan tspan = (DateTime.Now - old);
                    msTimeout = 250 - ((int)tspan.TotalMilliseconds);
                }
                catch (ThreadAbortException) { }
                catch (Exception e)
                {
                    Server.s.Log("Physics error on " + this.name + ": " + e.ToString());
                    msTimeout = -Server.Overload;
                }
                if (msTimeout <= -Server.Overload)
                {
                    this.m_physics = 0;
                    this.ClearPhysics();
                    Player.GlobalMessage("!PHYISCS OVERLOAD ON MAP: " + this.name + "!");
                    return;
                }
            }
        }
        ...
        public void Blockchange(Player p, ushort x, ushort y, ushort z, byte type, bool addaction)
        {
            try
            {
                ...
                if (!this.gamemode.Blockchange(p, x, y, z, b, type)) { p.SendBlockchange(x, y, z, b); return; }
                ...
                if (Block.Convert(b) != Block.Convert(type))    //Should save bandwidth sending identical looking blocks, like air/op_air changes.
                {
                    ...
                    if (plychanges[x, y, z] != pn)
                    {
                        plyblocks[x, y, z] = this.GetTile(x, y, z);
                        plychanges[x, y, z] = pn;
                    }
        ...
        public int Backup()
        {
            if (!this.backedup)
            {
                string destFileName = "levels/backups/" + this.name + "/";
                if (!Directory.Exists(destFileName)) Directory.CreateDirectory(destFileName);
                int lbackup = 0;
                try
                {
                    lbackup = Convert.ToInt32(File.ReadAllText(destFileName + "/LAST.txt"));
                }
                catch (Exception) { }
                lbackup++;
                File.WriteAllText(destFileName + "/LAST.txt", lbackup.ToString());
                destFileName += lbackup.ToString() + "/";
                if (!Directory.Exists(destFileName)) Directory.CreateDirectory(destFileName);
                destFileName += this.name + ".lvl";
                string sourceFileName = "levels/" + this.name + ".lvl";
                try
                {
                ...
                {
                    Server.s.Log("FAILED TO INCREMENTAL BACKUP :" + this.name);
                    Player.GlobalMessage("FAILED TO INCREMENTAL BACKUP :" + this.name);
                    return -1;
                }
            }
        ...
        public static Level Load(string name, byte phys)
        {
            ...
            if (File.Exists(path))
            {
                ...
                try
                {
                    ...
                    gs.Close(); 
                    //Server.s.Log("LOADED: Level \"" + name + "\"."); 
                    level.backedup = true;
                    return level;
                }
                catch (Exception ex) { Server.s.Log("ERROR loading level \"" + name + "\"."); Server.s.Log(ex.Message); Server.s.Log(ex.StackTrace); return null; }
                finally { fs.Close(); }
            }
		...
		public static void AutoDimension(ushort i)
		{
			if (i < 32)		{ i = 32;	return; }
			if (i < 64)		{ i = 64;	return; }
			if (i < 128)	{ i = 128;	return; }
			if (i < 256)	{ i = 256;	return; }
			if (i < 512)	{ i = 512;	return; }
