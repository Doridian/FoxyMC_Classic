    ...
    public sealed class Player
    {
        ...
        void HandleLogin(byte[] message)
        {
			try
			{
                ...
                this.group = Server.ranks.GetGroup(this);

				if (this.group.Permission <= LevelPermission.Banned) { Kick("You're banned!"); return; }
                ...
                ServerInterlink.PlayerJoined(this);
        ...
        void HandleBlockchange(byte[] message)
        {
            ...
            try
            {
                ...
                if (group.Permission == LevelPermission.Guest)
                {
                    ...
                    {
                        if (y < level.depth / 2 - Server.maxDepth)     //Anti tunneling countermeasure
                        {
        ...
        void HandleCommand(string cmd, string message)
        {
            ...
            if(Command.all.ContainsKey(cmd))
            {
                Command command = Command.all[cmd];
                if (group.CanExecute(command))
                {
                    Server.s.Log(name + " uses /" + cmd + " " + message);
        ...
        {
            if (showname) { ServerInterlink.PlayerChat(from, message); message = from.color + from.name + ": &f" + message; }
        ...
        public void Disconnect()
        {
            ...
            if (loggedIn)
            {
                ...
                ServerInterlink.PlayerLeft(this);
                players.Remove(this);
                Server.s.PlayerListUpdate();
                /*if (!Server.console && Server.win != null)
                    Server.win.UpdateClientList(players);*/
        ...
        public static Group GetGroup(string name)
        {
            ...
            return Server.ranks.GetGroup(name);
        }
        #endregion
        #region == OTHER ==
        static byte FreeId()
        ...
        public ushort NTHO(byte[] x, int offset)
        {
            byte[] y = new byte[2];
            Buffer.BlockCopy(x, offset, y, 0, 2); Array.Reverse(y);
            return BitConverter.ToUInt16(y, 0);
        }
        public byte[] HTNO(short x)
        {
            byte[] y = BitConverter.GetBytes(x); Array.Reverse(y); return y;
        }
        #endregion

        public bool CheckBlocklimit(int blockswanted)
        {
            if (this.group.Permission >= LevelPermission.Admin)
            {
                return (blockswanted < 20480);
            }
            else if (this.group.Permission >= LevelPermission.Operator)
            {
                return (blockswanted < 4096);
            }
            else
            {
                return (blockswanted < 1024);
            }
        }
		...
		bool CheckBlockSpam()
		{
			if (spamBlockLog.Count >= spamBlockCount)
			{
				...
				if (spamTimer < spamBlockTimer)
				{
					...
					Player.GlobalMessageOps(c.red + name + " builds too fast!");
